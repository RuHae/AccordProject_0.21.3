...
class Chaincode {
  ...
  async deploySmartLegalContract(stub, args) {
    ...
    //console.log('templateDate: ' + template)
  ...
  async executeSmartLegalContract(stub, args) {
    ...
    // execute the engine
    const engine = new Engine();
    const result = await engine.trigger(clause, request, state); // this is the original --> await engine.execute(clause, request, state);
    console.info(`Response from engine execute: ${JSON.stringify(result)}`);
